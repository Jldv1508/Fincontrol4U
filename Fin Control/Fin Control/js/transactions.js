  ...
  async function renderLoansList() {
    ...
    try {
      ...
      container.innerHTML = loans.map(l => {
        ...
        const realOutstanding = (Array.isArray(l.schedule) && l.schedule[doneInst])
          ? Number(l.schedule[doneInst].pendingAmount || 0)
          : remainingAmount;
        return `
        <div class="loan-item" data-id="${l.id}">
          ...
          <div class="loan-meta">
            ...
            <span>Saldo pendiente real: <strong>${formatAmount(realOutstanding)}</strong></span>
  ...
  function showLoanScheduleModal(loan) {
    ...
    const table = schedule.length ? schedule.map(r => `
      <tr>
        ...
        <td><button class="btn danger sched-delete" data-idx="${r.idx}"><i class="fas fa-trash"></i> Eliminar</button></td>
      </tr>
    ...
    const html = `
      <div class="loan-schedule">
        ...
        <p class="hint">Modifica fechas e importes de cada cuota. Puedes eliminar cuotas si es necesario. Los intereses, el capital y el pendiente se recalculan automáticamente.</p>
    ...
    if (typeof showModal === 'function') {
      showModal({
        ...
        size: 'large',
        onOpen: () => {
          // Eliminar cuota: borra la fila del DOM y reindexa
          document.querySelectorAll('.sched-delete').forEach(btn => {
            btn.addEventListener('click', () => {
              const tr = btn.closest('tr');
              if (tr && tr.parentElement) {
                tr.parentElement.removeChild(tr);
                Array.from(document.querySelectorAll('.amortization-table tbody tr')).forEach((row, idx) => {
                  const numCell = row.querySelector('td:first-child');
                  if (numCell) numCell.textContent = `#${idx + 1}`;
                  const dateInput = row.querySelector('.sched-date');
                  const amountInput = row.querySelector('.sched-amount');
                  const delBtn = row.querySelector('.sched-delete');
                  if (dateInput) dateInput.setAttribute('data-idx', String(idx + 1));
                  if (amountInput) amountInput.setAttribute('data-idx', String(idx + 1));
                  if (delBtn) delBtn.setAttribute('data-idx', String(idx + 1));
                });
              }
            });
          });
